...
void NetMonitor::UpdateOnce(monitor::proto::MonitorInfo* monitor_info) {
  ...
  while (net_file.ReadLine(&net_datas)) {
    ...
    if (name.find(':') == name.size() - 1 && net_datas.size() >= 10) {
      ...
      name.pop_back();
      net_info.name = name;
      net_info.rcv_bytes = std::stoull(net_datas[1]);
      net_info.rcv_packets = std::stoull(net_datas[2]);
      net_info.err_in = std::stoull(net_datas[3]);
      net_info.drop_in = std::stoull(net_datas[4]);
      net_info.snd_bytes = std::stoull(net_datas[9]);
      net_info.snd_packets = std::stoull(net_datas[10]);
      net_info.err_out = std::stoull(net_datas[11]);
      net_info.drop_out = std::stoull(net_datas[12]);
...
Inter-|   Receive                                                |  Transmit
 face |bytes    packets errs drop fifo frame compressed multicast|bytes    packets errs drop fifo colls carrier compressed
    lo: 85539541  565054    0    0    0     0          0         0 85539541  565054    0    0    0     0       0          0
 ens33: 175413190 2128064    0    0    0     0          0         0 2529378199 1994456    0    0    0     0       0          0
