  ...
  while (cpu_stat_file.ReadLine(&cpu_stat_list)) {
    if (cpu_stat_list[0].find("cpu") != std::string::npos) {
      ...
      if (it != cpu_stat_map_.end())
      {
        ...
        struct CpuStat old = std::move(it->second);
      ...
      //cpu_stat_map_里记录的是 系统文件存的信息 也就是monitor::CpuStatMonitor::CpuStat的信息
    }
    cpu_stat_list.clear();//每读一行，清除一次cpu_stat_list
  }
...
/proc/stat 文件存储格式：
    ...
    cpu  1000   200   300   4000  500   600    700   800   900    1000  // 整体 CPU
    cpu0 500    100   150   2000  250   300    350   400   450    500    // cpu 0
    cpu1 ...                                                             // cpu 1
*/