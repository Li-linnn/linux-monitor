...
void MemModel::UpdateMonitorInfo(
  ...
  // beginResetModel();和endResetModel();自动触发
  endResetModel();
  ...
  for (int i = MemInfo::USED_PERCENT; i < COLUMN_MAX; i++) {
    switch (i) {
      case MemInfo::USED_PERCENT:
        mem_info_list.emplace_back(QVariant(mem_info.used_percent()));
        break;
      case MemInfo::TOTAL:
        mem_info_list.emplace_back(QVariant(mem_info.total()));
        break;
      case MemInfo::FREE:
        mem_info_list.emplace_back(QVariant(mem_info.free()));
        break;
      case MemInfo::AVAIL:
        mem_info_list.emplace_back(QVariant(mem_info.avail()));
        break;
      case MemInfo::BUFFERS:
        mem_info_list.emplace_back(QVariant(mem_info.buffers()));
        break;
      case MemInfo::CACHED:
        mem_info_list.emplace_back(QVariant(mem_info.cached()));
        break;
      case MemInfo::SWAP_CACHED:
        mem_info_list.emplace_back(QVariant(mem_info.swap_cached()));
        break;
      case MemInfo::ACTIVE:
        mem_info_list.emplace_back(QVariant(mem_info.active()));
        break;
      case MemInfo::INACTIVE:
        mem_info_list.emplace_back(QVariant(mem_info.inactive()));
        break;
      case MemInfo::ACTIVE_ANON:
        mem_info_list.emplace_back(QVariant(mem_info.active_anon()));
        break;
      case MemInfo::INACTIVE_ANON:
        mem_info_list.emplace_back(QVariant(mem_info.inactive_anon()));
        break;
      case MemInfo::DIRTY:
        mem_info_list.emplace_back(QVariant(mem_info.dirty()));
        break;
      case MemInfo::WRITEBACK:
        mem_info_list.emplace_back(QVariant(mem_info.writeback()));
        break;
      case MemInfo::ANON_PAGES:
        mem_info_list.emplace_back(QVariant(mem_info.anon_pages()));
        break;
      case MemInfo::MAPPED:
        mem_info_list.emplace_back(QVariant(mem_info.mapped()));
        break;
      case MemInfo::KRECLAIMABLE:
        mem_info_list.emplace_back(QVariant(mem_info.kreclaimable()));
        break;
      case MemInfo::SRECLAIMABLE:
        mem_info_list.emplace_back(QVariant(mem_info.sreclaimable()));
        break;
      case MemInfo::SUNRECLAIM:
        mem_info_list.emplace_back(QVariant(mem_info.sunreclaim()));
        break;
      default:
        break;
    }
