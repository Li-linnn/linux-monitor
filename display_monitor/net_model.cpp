  ...
  for (int i = NetModelInfo::NAME; i < COLUMN_MAX; i++) {
    switch (i) {
      case NetModelInfo::NAME:
        net_info_list.emplace_back(QString::fromStdString(net_info.name()));
        break;
      case NetModelInfo::SEND_RATE:
        net_info_list.emplace_back(QVariant(net_info.send_rate()));
        break;
      case NetModelInfo::RCV_RATE:
        net_info_list.emplace_back(QVariant(net_info.rcv_rate()));
        break;
      case NetModelInfo::SEND_PACKETS_RATE:
        net_info_list.emplace_back(QVariant(net_info.send_packets_rate()));
        break;
      case NetModelInfo::RCV_PACKETS_RATE:
        net_info_list.emplace_back(QVariant(net_info.rcv_packets_rate()));
        break;
      default:
        break;
    }
