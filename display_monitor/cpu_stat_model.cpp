  ...
  for (int i = CpuStat::CPU_NAME; i < COLUMN_MAX; i++) {
    switch (i) {
      case CpuStat::CPU_NAME:
        // cpu_stat_list.push_back(QVariant(cpu_stat.cpu_name()));
        cpu_stat_list.emplace_back(QVariant(QString::fromStdString(cpu_stat.cpu_name())));
        break;
      case CpuStat::CPU_PERCENT:
        cpu_stat_list.emplace_back(QVariant(cpu_stat.cpu_percent()));
        break;
      case CpuStat::CPU_USER_PERCENT:
        cpu_stat_list.emplace_back(QVariant(cpu_stat.usr_percent()));
        break;
      case CpuStat::CPU_SYSTEM_PERCENT:
        cpu_stat_list.emplace_back(QVariant(cpu_stat.system_percent()));
        break;
      default:
        break;
    }
